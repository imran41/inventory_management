    ...
    if "started" not in st.session_state:
        ...
        if df is not None:
            ...
            with col1:
                ...
                # Percentage slider
                question_percentage = st.slider(
                    "📊 Select % of questions to attempt",
                    min_value=10,
                    max_value=100,
                    value=100,
                    step=10,
                    help="Choose what percentage of total questions you want to practice"
                )
            ...
            with col2:
                if show_timer_option:
                    ...
                    num_questions = max(1, int(len(df) * question_percentage / 100))
                    total_time = num_questions * time_per_q
                    st.info(f"Questions to attempt: {num_questions} of {len(df)}")
                    st.info(f"Total exam time: {format_time(total_time)}")
                else:
                    num_questions = max(1, int(len(df) * question_percentage / 100))
                    total_time = 999999  # Unlimited time
            ...
            if st.button("🚀 Start Exam", type="primary", use_container_width=True):
                processed_df = shuffle_questions(df) if shuffle else df
                # Select only the percentage of questions chosen
                num_questions = max(1, int(len(processed_df) * question_percentage / 100))
                processed_df = processed_df.head(num_questions).reset_index(drop=True)
                init_session(processed_df, total_time)
                st.rerun()
        else:
            st.error("Failed to load the CSV file. Please check the file path and format.")
